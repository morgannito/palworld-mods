...
Server-side datatable mods for Palworld 1.0 dedicated servers (Linux, pure
`~mods` paks, **zero UE4SS**) plus the full reproducible pipeline to build
them: extract → JSON → Python patch → repack.

Server-side only: drop a pak on the server, restart — nothing to install on
clients (crossplay console players included).
